...
查宝（药店版）是一款面向医保药店数据筛查，基于 LangGraph 构建的多节点决策智能体，具备工具调用（Tool Calling）与人在回路（Human-in-the-Loop）能力。它通过简洁的网页对话界面，引导用户完成列名配置、退货冲销检查、销售端与医保端比对、处方端与医保端比对等工作，并将结果文件生成在本地工作目录中。
